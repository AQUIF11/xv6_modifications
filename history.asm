...
int main(void) {
  ...
  1d:	6a 40                	push   $0x40
  1f:	50                   	push   %eax
  20:	e8 5e 03 00 00       	call   383 <gethistory>

    if (count < 0) {
  25:	83 c4 10             	add    $0x10,%esp
  28:	85 c0                	test   %eax,%eax
  2a:	78 4c                	js     78 <main+0x78>
  ...
  35:	6a 01                	push   $0x1
  37:	e8 14 04 00 00       	call   450 <printf>
    for (int i = 0; i < count; i++) {
  3c:	83 c4 10             	add    $0x10,%esp
  3f:	85 ff                	test   %edi,%edi
  41:	74 30                	je     73 <main+0x73>
  43:	8d 9d ec f9 ff ff    	lea    -0x614(%ebp),%ebx
  ...
  53:	ff 73 10             	push   0x10(%ebx)
    for (int i = 0; i < count; i++) {
  56:	83 c6 01             	add    $0x1,%esi
        printf(1, "%d\t%s\t%d\n", hist[i].pid, hist[i].name, hist[i].mem_usage);
  59:	53                   	push   %ebx
    for (int i = 0; i < count; i++) {
  5a:	83 c3 18             	add    $0x18,%ebx
        printf(1, "%d\t%s\t%d\n", hist[i].pid, hist[i].name, hist[i].mem_usage);
  5d:	ff 73 e4             	push   -0x1c(%ebx)
  60:	68 ac 07 00 00       	push   $0x7ac
  65:	6a 01                	push   $0x1
  67:	e8 e4 03 00 00       	call   450 <printf>
    for (int i = 0; i < count; i++) {
  6c:	83 c4 20             	add    $0x20,%esp
  6f:	39 f7                	cmp    %esi,%edi
  71:	75 dd                	jne    50 <main+0x50>
    }

    exit();
  73:	e8 6b 02 00 00       	call   2e3 <exit>
        printf(1, "Error fetching process history\n");
  78:	52                   	push   %edx
  79:	52                   	push   %edx
  7a:	68 78 07 00 00       	push   $0x778
  7f:	6a 01                	push   $0x1
  81:	e8 ca 03 00 00       	call   450 <printf>
        exit();
  86:	e8 58 02 00 00       	call   2e3 <exit>
  8b:	66 90                	xchg   %ax,%ax
  8d:	66 90                	xchg   %ax,%ax
  8f:	90                   	nop
 ...
 3ac:	89 d1                	mov    %edx,%ecx
{
 ...
 3d9:	0f b6 92 18 08 00 00 	movzbl 0x818(%edx),%edx
 ...
 459:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 4c9:	ff 24 85 c0 07 00 00 	jmp    *0x7c0(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
 ...
 5d3:	2e 8d 74 26 00       	lea    %cs:0x0(%esi,%eiz,1),%esi
          s = "(null)";
 5d8:	ba b6 07 00 00       	mov    $0x7b6,%edx
        while(*s != 0){
...
free(void *ap)
{
 ...
 5f1:	a1 cc 0a 00 00       	mov    0xacc,%eax
{
 ...
 631:	5b                   	pop    %ebx
  freep = p;
 632:	89 15 cc 0a 00 00    	mov    %edx,0xacc
}
 638:	5e                   	pop    %esi
 639:	5f                   	pop    %edi
 63a:	5d                   	pop    %ebp
 ...
 669:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 66c:	89 15 cc 0a 00 00    	mov    %edx,0xacc
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 689:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 68c:	8b 3d cc 0a 00 00    	mov    0xacc,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 6cf:	73 4f                	jae    720 <malloc+0xa0>
    ...
    if(p == freep)
 6d1:	8b 3d cc 0a 00 00    	mov    0xacc,%edi
 ...
 6fa:	e8 f1 fe ff ff       	call   5f0 <free>
  return freep;
 6ff:	8b 15 cc 0a 00 00    	mov    0xacc,%edx
 ...
 747:	0a 00 00 
    base.s.size = 0;
 74a:	bf d0 0a 00 00       	mov    $0xad0,%edi
    base.s.ptr = freep = prevp = &base;
 ...
 759:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 75b:	c7 05 d4 0a 00 00 00 	movl   $0x0,0xad4
 762:	00 00 00 
    if(p->s.size >= nunits){
